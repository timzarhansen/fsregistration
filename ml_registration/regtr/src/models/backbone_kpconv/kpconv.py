...
# GPU dependencies (optional - only needed for PreprocessorGPU)
try:
...
# CPU operations for KPConv preprocessing
...
class PreprocessorGPU(torch.nn.Module):
    ...
    Requires MinkowskiEngine and pytorch3d to be installed.
    """

    def __init__(self, cfg):
        if not GPU_AVAILABLE:
            raise ImportError(
                "PreprocessorGPU requires MinkowskiEngine and pytorch3d. "
                "Either install these packages or use Preprocessor for CPU mode."
            )
        super().__init__()
        self.cfg = cfg
