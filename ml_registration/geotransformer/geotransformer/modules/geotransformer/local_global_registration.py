...
class LocalGlobalRegistration(nn.Module):
    ...
    def convert_to_batch(ref_corr_points, src_corr_points, corr_scores, chunks):
        ...
        indices = torch.cat([torch.arange(x, y, device=device) for x, y in chunks], dim=0)
        ...
        indices = torch.cat([torch.arange(x, y, device=device) for x, y in target_chunks], dim=0)
