...
def to_cuda(x):
    r"""Move all tensors to cuda. Deprecated: use to_device() instead."""
    return to_device(x, device='cuda')


def to_device(x, device=None):
    r"""Move all tensors to specified device.

    Args:
        x: tensor, list, tuple, or dict of tensors
        device: torch.device or str. If None, keeps tensors on their current device.
    """
    if isinstance(x, list):
