...
def fuse_images(imagesOverTime, estimatedTransformations):
    ...
    for i in range(len(imagesOverTime)):
        ...
        corners = np.array([[0, 0], [w, 0], [0, h], [w, h]], dtype=np.float32).reshape(-1, 1, 2)  # Reshape to Nx1x2
    ...
    if warped_images:
        stacked = np.stack(warped_images).astype(np.float32)
...
def normalize_image(image):
    normalized_array = image.astype(np.float32).reshape(-1)
...
if __name__ == '__main__':
    ...
    timestamps_path = os.path.join(os.path.join(radar_file_path, os.pardir, 'radar.timestamps'))
    if not os.path.isfile(timestamps_path):
        raise IOError("Could not find timestamps file")
    gt_paths = os.path.join(os.path.join(radar_file_path, os.pardir, 'gt/radar_odometry.csv'))
    if not os.path.isfile(gt_paths):
        raise IOError("Could not find GT file")


    pathToSave = f"saveRandomImagesOxford/{N}_{int(size_of_pixel*100)}_{matching_every_nth_image}/"
    ...
    with open(gt_paths, 'r') as file:
        ...
        estPoseList = []
        #getting first image
        filename = os.path.join(args.dir, str(radar_timestamps[0]) + '.png')
        firstImage = get_image_from_path(filename,N,size_of_pixel,True)
        firstIndex= 0
        cv2.imwrite(pathToSave + f"original_image_{firstIndex:04d}.png", firstImage * 255.0)
        imagesOverTime.append(firstImage*255.0)
        estimatedTransformationList.append(np.eye(4))
        absoluteTransformationListForImagePrint.append(np.eye(4))
        ...
        for index , radar_timestamp in enumerate(radar_timestamps):
            ...
            absoluteTransformation = np.matmul(transformation_matrix_gt,absoluteTransformation)
            absoluteTransformationList.append(absoluteTransformation)

            if index>matching_every_nth_image-1:
                if index % matching_every_nth_image==0:
                    currentGtRow = gt_data[startingRowIndex[0]+index]
                    transformation_matrix_gt = np.matmul(np.linalg.inv(absoluteTransformationList[index-matching_every_nth_image]),absoluteTransformationList[index])

                    filename2 = os.path.join(args.dir, str(radar_timestamp) + '.png')
                    cart_img2 = get_image_from_path(filename2,N,size_of_pixel,True)
                    cv2.imwrite(pathToSave + f"original_image_{index:04d}.png", cart_img2 * 255.0)
                    filename1 = os.path.join(args.dir, str(radar_timestamps[index-matching_every_nth_image]) + '.png')
                    cart_img1 = get_image_from_path(filename1,N,size_of_pixel,True)
                    imagesOverTime.append(cart_img2*255.0)
                    ...
                    estPoseList.append(getxyYaw(perspectiveMatrixEstimated))
        ...
        print(len(imagesOverTime))
        print(len(absoluteTransformationList))
        print(len(absoluteTransformationListForImagePrint))
        print("done")
        # fused = fuse_images(imagesOverTime,absoluteTransformationListForImagePrint)
        # cv2.imwrite(pathToSave+f"blended_fullMapGT.png", fused*5)
        # fused = fuse_images(imagesOverTime,estimatedTransformationList)
