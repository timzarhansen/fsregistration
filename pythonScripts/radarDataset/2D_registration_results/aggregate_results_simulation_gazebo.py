#!/usr/bin/env python3
"""
Aggregate benchmark results for the simulation gazebo 2D radar dataset.
...
For subdirectories with N >= 256 only:
  - Groups all runs (sequence x noise level) by registration method
  - Computes aggregate error statistics per method
  - Writes aggregated_simulation_gazebo_results.csv

Directory naming: seq01_<method>_N256_p23_s1[<noise_suffix>] where
noise_suffix is one of _high, _low, _high_gauss, _high_salt_pepper,
_low_gauss, _low_salt_pepper (runs without a suffix use 'base').
Directories without a results.csv (e.g. methods still processing)
are skipped and picked up automatically once results appear.

Usage:
    python aggregate_results_simulation_gazebo.py
"""
...
# Configuration
# ============================================================================
INPUT_FOLDER = Path(__file__).resolve().parent / "ResultsRadar_DFKI" / "simulation_gazebo_scans"
OUTPUT_PATH = INPUT_FOLDER / "aggregated_simulation_gazebo_results.csv"
OUTPUT_PATH_ALL = INPUT_FOLDER / "aggregated_simulation_gazebo_results_all.csv"
...
def main():
    ...
    all_results = []   # one row per subdirectory (→ aggregated_simulation_gazebo_results_all.csv)
    method_data = {}   # method_name -> pooled error values (→ aggregated_simulation_gazebo_results.csv)
    ...
    # ── Write per-subdirectory file (aggregated_simulation_gazebo_results_all.csv) ──
    all_columns = ["dir_name", "seq", "method", "N", "p", "s", "noise",
    ...
    # ── Write per-method file (aggregated_simulation_gazebo_results.csv) ──
    results = []
