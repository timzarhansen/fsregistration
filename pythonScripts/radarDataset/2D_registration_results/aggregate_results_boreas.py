#!/usr/bin/env python3
"""
Aggregate benchmark results for the Boreas 2D radar dataset.
...
For subdirectories with N >= 256 only:
  ...
  - Writes aggregated_boreas_results.csv

Usage:
    python aggregate_results_boreas.py
"""
...
# Configuration
# ============================================================================
INPUT_FOLDER = Path(__file__).resolve().parent / "ResultsRadar_DFKI" / "boreas2d"
OUTPUT_PATH = INPUT_FOLDER / "aggregated_boreas_results.csv"
OUTPUT_PATH_ALL = INPUT_FOLDER / "aggregated_boreas_results_all.csv"
...
def main():
    ...
    all_results = []   # one row per subdirectory (→ aggregated_boreas_results_all.csv)
    method_data = {}   # method_name -> pooled error values (→ aggregated_boreas_results.csv)
    ...
    # ── Write per-subdirectory file (aggregated_boreas_results_all.csv) ──
    ...
    # ── Write per-method file (aggregated_boreas_results.csv) ──
    results = []
