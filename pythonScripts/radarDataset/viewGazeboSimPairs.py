"""
Gazebo-Sim Pair Viewer - Automated pair-by-pair registration with image saving

Plots registration results for the simulated LiDAR dataset
(2D-Scan-Gazebo-Dataset, recorded with scan_data_gazebo_sim, maze worlds).

Usage:
    python viewGazeboSimPairs.py

Saves display images to viewGazeboSimOutput/ folder:
    image1.png - previous scan
    image2.png - current scan
...
from lidarSimDatasetLoader import LidarSimSequence, list_sequences
...
NOISE_LEVEL = "high"  # Options: None, low, high, low_gauss, high_gauss, low_salt_pepper, high_salt_pepper
RENDER_MODE = "binary"  # Options: binary (no blur), gaussian (light blur), average (old pipeline)
...
def get_config_from_file():
    ...
    config_path = os.path.join(source_dir, "viewGazeboSimPairs.py")
    ...
    NOISE_LEVEL = extract_var("NOISE_LEVEL", NOISE_LEVEL)
    N = extract_var("N", N)
...
PLOT_DATA_DIR = "/home/tim-external/ros_ws/src/fsregistration/plotting_results/2d/data_gazebosim"
...
def get_lidarsim_affine(transform_se3: np.ndarray,
                          pixel_size: float = 1.0,
                          img_size: int = 0) -> np.ndarray:
    """Convert standard SE3 transform to image warp affine for Gazebo sim.

    LidarSim images map local-frame (lx, ly) directly to pixel (gx, gy):
...
def run_pair(
    ...
    fs2d_affine = get_lidarsim_affine(result.transform, pixel_size=SIZE_OF_PIXEL, img_size=N)
...
def main():
    ...
    if not sequences:
        raise FileNotFoundError(f"No gazebo-sim dataset found in {DATA_DIR}")
    if SEQUENCE_NUMBER < 1 or SEQUENCE_NUMBER > len(sequences):
        raise ValueError(
            f"SEQUENCE_NUMBER {SEQUENCE_NUMBER} out of range "
    ...
    length_of_scans = seq.length
    if MAX_FRAMES is not None:
        length_of_scans = min(length_of_scans, MAX_FRAMES)
    print(f"Matching every {MATCHING_STEP}th scan (from scan {START_FRAME}, up to {length_of_scans})")
