"""
EfficientLoFTR Parameter Sweep Benchmark

Tests all combinations of configurable EfficientLoFTR parameters on a single
Boreas sequence (default: seq 0). Each combination gets its own output
subdirectory with results.csv and config.json.

Usage:
    python boreasBenchmarkEfficientLoFTRSweep.py
"""
...
PARAM_ABBREV = {
    "eloftr_model_type": "type",
    "eloftr_ransac_threshold": "rth",
    "eloftr_ransac_confidence": "rcf",
    "round": "mask",
    "radius": "R",
    "matching_step": "step",
    "N": "N",
...
def build_output_name(combo: dict, varied_keys: List[str], N: int,
    ...
    parts = [f"seq{SEQUENCE:02d}_eloftr_N{N:03d}_p{int(size_of_pixel * 100):04d}{s_part}"]
...
def run_single_combo(args: tuple) -> Tuple[str, dict]:
    ...
    method_config = {
        ...
        "eloftr_model_type": combo["eloftr_model_type"],
        "eloftr_ransac_threshold": combo["eloftr_ransac_threshold"],
        "eloftr_ransac_confidence": combo["eloftr_ransac_confidence"],
    }
    ...
    method = RegistrationFactory.create("eloftr", method_config)
...
def main():
    ...
    print("=" * 75)
    print("EfficientLoFTR Parameter Sweep Benchmark")
    print("=" * 75)
