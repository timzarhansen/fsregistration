"""
Fourier-Mellin Parameter Sweep Benchmark

Tests all combinations of configurable Fourier-Mellin parameters on a single
Boreas sequence (default: seq 0).  Each combination gets its own output
subdirectory with results.csv and config.json in the same format as
boreasBenchmark.py.

Usage:
    python boreasBenchmarkFourierMellinSweep.py
"""
...
def build_output_name(combo: dict, varied_keys: List[str], N: int,
    ...
    parts = [f"seq{SEQUENCE:02d}_fm_N{N:03d}_p{int(size_of_pixel * 100):04d}{s_part}"]
...
def run_single_combo(args: tuple) -> Tuple[str, dict]:
    ...
    method = RegistrationFactory.create("fourier_mellin", method_config)
...
def main():
    ...
    print("=" * 75)
    print("Fourier-Mellin Parameter Sweep Benchmark")
    print("=" * 75)
