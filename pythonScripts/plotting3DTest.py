...
if __name__ == "__main__":
    ...
    draw_pcl_no_color_change(pcd1, np.identity(4), 0)
    draw_pcl_no_color_change(pcd2, np.identity(4), 1)

    # pcd1 = o3d.io.read_point_cloud(demo_colored_icp_pcds.paths[0])
    # pcd1.colors = np.asarray(pcd1.colors)*255.0
    # pcd2 = o3d.io.read_point_cloud(demo_colored_icp_pcds.paths[1])

    # pcd1.colors = o3d.utility.Vector3dVector(np.asarray(pcd1.colors)*255.0)
    # pcd2.colors = o3d.utility.Vector3dVector(np.asarray(pcd2.colors)*255.0)
    # pcd2.colors = pcd2.colors * 255.0
    # sourcePoints = np.asarray(source.points)
    # sourceColor = np.asarray(source.colors)
    # sourcePointsRGB = np.hstack([sourcePoints,sourceColor])
    ...
    set_r_manual = True
    level_potential_rotation = 0.001  # 0.01 , 0.001
    level_potential_translation = 0.01  # 0.1 , 0.01
    ...
    # print(response)
    draw_registration_result_no_color_change(pcd1, pcd2, T)  # GT
    draw_registration_result_no_color_change(pcd1, pcd2, estimatedActualRotation1)  # Estimation
